# -*- coding: utf-8 -*-
"""
Ollama + LangChain System for Local Ubuntu Server
Modified from Google Colab version for local deployment
"""
...
def check_and_install_packages():
    """Check and install required packages for local Ubuntu environment"""
    required_packages = [
        "langchain",
        "langchain-community", 
        "langchain-core",
...
class OllamaConfig:
    ...
    gpu_layers: int = -1  # -1 for all GPU layers, 0 for CPU only
...
class OllamaManager:
    """Ollama manager for local Ubuntu environment"""
    ...
    def install_ollama(self):
        """Install Ollama on local Ubuntu system"""
        ...
        try:
            ...
            print("Try manual installation: curl -fsSL https://ollama.com/install.sh | sh")
            raise
    ...
    def start_server(self):
        ...
        try:
            ...
            if self.config.use_gpu:
                env["OLLAMA_GPU_LAYERS"] = str(self.config.gpu_layers)
                
            env["OLLAMA_NUM_PARALLEL"] = str(self.config.threads)
            env["OLLAMA_MAX_LOADED_MODELS"] = "3"
            env["OLLAMA_FLASH_ATTENTION"] = "1"  # Enable flash attention for better performance
...
class PerformanceMonitor:
    ...
    def _monitor_loop(self):
        ...
        while self.monitoring:
            try:
                ...
                # Try to get GPU usage if nvidia-smi is available
                try:
                    result = subprocess.run(
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if self.stats["gpu_usage"]:
            stats["avg_gpu"] = sum(self.stats["gpu_usage"][-10:]) / max(len(self.stats["gpu_usage"][-10:]), 1)
...
class RAGSystem:
    ...
    def __init__(self, llm: OllamaLLM, embedding_model: str = "sentence-transformers/all-MiniLM-L6-v2"):
        ...
        self.embeddings = HuggingFaceEmbeddings(
            ...
            model_kwargs={'device': 'cuda' if self._gpu_available() else 'cpu'}
        )
...
class OllamaLangChainSystem:
    ...
    def setup(self):
        ...
        print("🚀 Setting up Ollama + LangChain system for Ubuntu...")
...
def main():
    ...
    try:
        ...
        print("\n🤖 Testing agent with search:")
        agent_response = system.agent_chat("What's the latest news about AI?")
...
def create_gradio_interface(system: OllamaLangChainSystem):
    ...
    try:
        ...
        with gr.Blocks(title="Ollama + LangChain Local System") as demo:
            gr.Markdown("# 🦙 Ollama + LangChain System (Local Ubuntu)")
...
if __name__ == "__main__":
    print("🚀 Ollama + LangChain System for Local Ubuntu Server")
    print("=" * 55)

    main()

    # Uncomment to run interactive Gradio interface
    # config = OllamaConfig(model_name="llama2", use_gpu=True)
    # system = OllamaLangChainSystem(config)
    # if system.setup():
